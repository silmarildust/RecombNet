...
def _scan_single(args: argparse.Namespace) -> None:
    labels, seqs, meta = load_alignment_file(
        args.alignment,
    ...
    if positions is not None and len(positions) != len(seqs[0]):
        raise SystemExit(
            f"positions file has {len(positions)} entries, but the alignment has {len(seqs[0])} columns"
        )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    scan = sub.add_parser("scan", help="analyse a single aligned FASTA or CSV file")
    scan.add_argument("alignment", help="aligned FASTA or Nextclade CSV file")
    scan.add_argument("-o", "--output", default="out", help="output prefix (default: out)")
    scan.add_argument("--reference", default="", help="reference FASTA used when a CSV has no explicit aligned-sequence column")
    scan.add_argument("--sequence-column", default="", help="name of the explicit aligned-sequence column in CSV inputs")
    ...
    arg.add_argument("--reference", default="", help="reference FASTA used when CSV files do not include explicit aligned sequences")
    arg.add_argument("--sequence-column", default="", help="name of the explicit aligned-sequence column in CSV inputs")
