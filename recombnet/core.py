...
def _sniff_delimited_file(path: str | os.PathLike[str]) -> tuple[csv.Dialect, str]:
    with open(path, "r", encoding="utf-8") as f:
        sample = f.read(8192)
    try:
    ...
    return dialect, first_line



def infer_filename_metadata(path: str | os.PathLike[str]) -> dict[str, str]:
    """Infer lineage/location metadata from LINEAGE_LOCATION.csv file names."""
    stem = Path(path).stem
    if "_" in stem:
        lineage, location = stem.split("_", 1)
    else:
        lineage, location = stem, ""
    return {
        "group_id": stem,
        "lineage": lineage,
...
def load_reference_fasta(path: str | os.PathLike[str]) -> str:
    """Load the first sequence from a reference FASTA file."""
    _, seqs = load_fasta(path)
    return seqs[0]



def _mutation_tokens(value: str | None) -> list[str]:
    if not value:
        return []
...
def _apply_nextclade_mutations(seq: list[str], row: dict[str, str]) -> list[str]:
    """Best-effort reconstruction from Nextclade mutation columns."""
    out = seq[:]
    substitution_re = re.compile(r"^([ACGTURYKMSWBDHVN-])([0-9]+)([ACGTURYKMSWBDHVN-])$", re.I)
    deletion_patterns = [
        re.compile(r"^([0-9]+)-([0-9]+)del$", re.I),
        re.compile(r"^del([0-9]+)-([0-9]+)$", re.I),
        re.compile(r"^del([0-9]+)_([0-9]+)$", re.I),
        re.compile(r"^([0-9]+)del$", re.I),
    ]
    missing_patterns = [
    ...
    for token in _mutation_tokens(row.get("substitutions")):
        m = substitution_re.match(token)
        if m:
            pos = int(m.group(2)) - 1
            alt = m.group(3).upper()
            if 0 <= pos < len(out):
                out[pos] = alt
    for token in _mutation_tokens(row.get("deletions")):
        token = token.lower().replace("deletion", "del")
        for pat in deletion_patterns:
            m = pat.match(token)
            if m:
                start = int(m.group(1)) - 1
                end = int(m.group(2)) - 1 if len(m.groups()) > 1 else start
                for pos in range(max(0, start), min(len(out), end + 1)):
                    out[pos] = "-"
                break
    for token in _mutation_tokens(row.get("missing")):
...
def load_alignment_csv(
    path: str | os.PathLike[str],
    *,
    reference_path: str | os.PathLike[str] | None = None,
    sequence_column: str | None = None,
) -> tuple[list[str], list[str], dict[str, object]]:
    """Load a Nextclade-style CSV/TSV file and return labels, sequences, and metadata."""
    dialect, _ = _sniff_delimited_file(path)
    with open(path, "r", encoding="utf-8") as f:
    ...
    first_row = rows[0]
    normalized = {str(k).strip().lower(): k for k in first_row.keys() if k is not None}
    candidates = [
        sequence_column,
        "alignedsequence",
        "aligned_sequence",
    ...
    for candidate in candidates:
        if candidate and candidate.lower() in normalized:
            seq_key = normalized[candidate.lower()]
            break
    ...
    if seq_key is not None:
        seqname_key = normalized.get("seqname", "seqName")
        for row in rows:
            label = str(row.get(seqname_key, "")).strip() or str(row.get(seq_key, "")).strip()
            seq = str(row.get(seq_key, "")).replace(" ", "").upper()
            if not seq:
                continue
            labels.append(label)
            seqs.append(seq)
        ...
        if reference_path is None:
            raise ValueError(
                f"CSV file {path} does not contain an explicit sequence column. "
                "Provide --reference to reconstruct an alignment from Nextclade mutation annotations."
            )
        ref = load_reference_fasta(reference_path)
        seqname_key = normalized.get("seqname", "seqName")
        for row in rows:
            label = str(row.get(seqname_key, "")).strip() if seqname_key else ""
            if not label:
                label = str(row.get(next(iter(first_row.keys())), "")).strip()
            seq_list = _apply_nextclade_mutations(list(ref), {str(k): ("" if v is None else str(v)) for k, v in row.items()})
            labels.append(label)
            seqs.append("".join(seq_list))

    if not seqs:
        raise ValueError(f"No usable aligned sequences found in {path}")
    length = len(seqs[0])
    if any(len(s) != length for s in seqs):
        raise ValueError(f"Sequences in {path} are not aligned to the same length")

    meta = infer_filename_metadata(path)
    observed_lineages = sorted(
        {
            str(row.get(normalized.get("nextclade_pango", "Nextclade_pango"), "")).strip()
            for row in rows
            if str(row.get(normalized.get("nextclade_pango", "Nextclade_pango"), "")).strip()
        }
    )
    meta.update(
        {
...
def load_alignment_file(
    ...
    *,
    ...
    if suffix in {".fa", ".fasta", ".fna", ".fas"}:
        labels, seqs = load_fasta(path)
        meta = infer_filename_metadata(path)
        meta.update({"num_records": len(seqs), "observed_lineages": []})
        return labels, seqs, meta
    if suffix in {".csv", ".tsv", ".txt"}:
        return load_alignment_csv(path, reference_path=reference_path, sequence_column=sequence_column)
    raise ValueError(f"Unsupported alignment file type: {path}")
...
def load_manifest(path: str | os.PathLike[str]) -> list[dict[str, object]]:
    ...
    The manifest may include either explicit metadata columns or just file paths.
    Supported columns: file/path, location, lineages, group_id. If location or lineages
    are missing, they can be inferred from filenames like LINEAGE_LOCATION.csv.
    """
    path = str(path)
    with open(path, "r", encoding="utf-8") as f:
        sample = f.read(4096)
    ...
    for idx, row in enumerate(rows):
        ...
        if not file_key:
            raise ValueError(f"Manifest row {idx+2} must contain a file/path column")
        inferred = infer_filename_metadata(file_key)
        location = norm.get("location") or norm.get("country") or norm.get("site") or inferred["location"]
        lineages = norm.get("lineages") or norm.get("lineage") or norm.get("pango") or inferred["lineage"]
        group_id = norm.get("group_id") or norm.get("id") or inferred["group_id"]
        out.append(
            {
...
def analyze_group(
    ...
    *,
    ...
    labels, seqs, meta = load_alignment_file(fasta_path, reference_path=reference_path, sequence_column=sequence_column)
    if group_id is None:
        group_id = str(meta.get("group_id", Path(fasta_path).stem))
    if location is None:
        location = str(meta.get("location", "") or Path(fasta_path).stem)
    if lineages is None:
        lineages = meta.get("observed_lineages", []) or []
